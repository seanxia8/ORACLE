...
PROBE_MARKER="$SCRIPT_DIR/full_training_probe_topas.ok"
ACTION="${1:-submit}"
...
case "$ACTION" in
    probe)
        ...
        rm -f "$PROBE_MARKER"
        submit_dry_run "$PROBE_JDL" /tmp/train_full_topas_probe.ad
    ...
    submit)
        ...
        ensure_wandb
        ensure_dataset_visible
        require_recent_probe "$PROBE_MARKER" topas
        submit_dry_run "$JDL" /tmp/train_full_topas.ad
        condor_submit "$JDL"
        ;;
    dry-run)
        submit_dry_run "$JDL" /tmp/train_full_topas.ad
