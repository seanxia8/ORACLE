...
configuration with a released checkpoint. The layerwise arm hooks all four
DynEdge convolution blocks plus the post-processing and readout stages, not
only the 128-dimensional pre-head event representation. Module dropout, hit
thinning, and time jitter instantiate N; S windows use clean low-density strata
in energy, topology, vertex/containment, and multiplicity, estimated from
training data only --- with the caveat that module loss \emph{lowers} observed
multiplicity, so every N cell is matched by an S cell of clean events at the
same observed multiplicity, charge, and time spread; otherwise attribution
reduces to recognizing the corruption operator's fingerprint. One secondary
architecture serves as a consistency check. The released NuBench datasets are
independent per-geometry productions and share no latent events, so the paired
detector-geometry contrast is constructed by re-simulation:
Prometheus~\cite{prometheus} supports re-using one seeded injection across
detector geometries, and the six NuBench geometry files ship with it. Until that paired set exists, the released
Hexagon water/ice pair supports only a distribution-level medium comparison,
...
symmetry, and adds declared artifacts. The public TIDMAD~\cite{tidmad} arm uses a
